...
**初始化：**在A [j..n]中A[ j ]为当前最小元素。由于此时处于数组尾部，所以初始化时不包含任何元素。<br/>
**保持：** 每次比较A[ j ]和A[ j - 1 ]，如果A[ j ] < A[ j - 1 ]那么交换他们两个的位置，此时子数组A[ j..n]都是由原来在A[ j..n ]中的元素组成的，且A[ j ]为当前子数组最小元素。for循环的下一次迭代减小 j 将保持循环不变式。<br/>
**终止：**当j<i+1时循环终止，因为每次循环迭代 j 减小1，那么必有j=i,在循环不变式的表述中将 j 用 i 代替，我们有：子数组A[i+1..n]由原来在A[i+1..n]中的元素组成，且A[ j ]为子数组的最小元素 。<br/>
